...
(function($) {
    ...
    */
                ...
                searchField.on('keyup paste', function() {
                    ...
                    keyPressTimeout = setTimeout( function() {
                        ...
                        if(searchTerm.length < 3) {
                            ...
                            var ajaxData = {
                                action: 'entropia_edge_search_post_types',
                                term: searchTerm,
                                postType: postType
                            };
    ...
    /**
     * Add more images for glitch effect
     */
    function edgtfGlitchify() {
    	var i,
			curHolder = $('.edgtf-glitch');

    	curHolder.each(function () {

    		var curImg = $(this).find('img'),
                curParent = curImg.parent();

            for (i=1; i<3; i++) {
                curImg.clone().appendTo(curParent);
            }
        })
    }

    function edgtfGlitchifyTxt() {

            var curHolder = $('.edgtf-glitch-txt'),
                child = curHolder.find('h1,h2,h3,h4,p, span');

        child.each(function (i) {

        	var childContent = child.get(i).innerText;
        	console.log(childContent);
            $(this).append('<span class="edgtf-glitch-after">'+ childContent +'</span>');
        })
    }
    ...
    /**
	 * Add sound to button on hover
     */
    function edgtfButtonSoundInit() {

		if($(document).find('.edgtf-btn')) {
			$('<audio class="edgtf-bttn-click-sound"></audio>').attr({
				'src': edgtfGlobalVars.vars.themeuri + '/assets/sound/bttn-click-sound.mp3',
			}).appendTo("body");
        }

        var audio = $('.edgtf-bttn-click-sound')[0];
        $('.edgtf-btn, .edgtf-btn-sound').click(function () {
            setTimeout(function () {
                audio.play();
        	}, 10);​
        });​
	...
	function edgtfInitBlogPagination(){
		...
		var initMainPagFunctionality = function(thisHolder) {
			...
			if(nextPage <= maxNumPages){
				...
				var ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'entropia_edge_blog_load_more');
...
(function($) {
    ...
    */
    ...
    /*
     ** Replace wrapper h2 with h4 for Related products
     */
    function edgtfReplaceRelatedTitle() {
        $('.related.products h2').replaceWith(function() {
    ...
    function edgtfInitBlogListShortcodePagination(){
        ...
        var initMainPagFunctionality = function(thisHolder, pagedLink) {
            ...
            if(nextPage <= maxNumPages){
                ...
                var ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'entropia_edge_blog_shortcode_load_more');
		...
		var buttonHoverBgColor = function(button) {
			if(typeof button.data('hover-bg-color') !== 'undefined') {
				...
				var changeButtonBg = function(event) {
                    if(event.data.button.hasClass('edgtf-btn-animation-3d')) {
                        event.data.button.find('.edgtf-btn-background-holder').css('background-color', event.data.color);
                    } else {
                        event.data.button.css('background-color', event.data.color);
                    }
				};

                if(button.hasClass('edgtf-btn-animation-3d')) {
                    originalBgColor = button.find('.edgtf-btn-background-holder').css('background-color');
                } else {
                    originalBgColor = button.css('background-color');
                }
		...
		var buttonHoverBorderColor = function(button) {
			if(typeof button.data('hover-border-color') !== 'undefined') {
				...
				var changeBorderColor = function(event) {
                    if(event.data.button.hasClass('edgtf-btn-animation-3d')) {
                        event.data.button.find('.edgtf-btn-background-holder').css('border-color', event.data.color);
                    } else {
                ...
                if(button.hasClass('edgtf-btn-animation-3d')) {
                    originalBorderColor = button.find('.edgtf-btn-background-holder').css('borderTopColor'); //take one of the four sides
                } else {
                    originalBorderColor = button.css('borderTopColor');
                }
...
(function($) {
    ...
    function edgtfInitLoadMoreItemsPortfolioFilter($portfolioList, $filterValue, $filterClassName) {
        ...
        var	loadMoreDatta = edgtf.modules.common.getLoadMoreData(thisPortList),
            nextPage = loadMoreDatta.nextPage,
	        ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'entropia_core_portfolio_ajax_load_more'),
	...
	function edgtfInitPortfolioPagination(){
		...
		var initMainPagFunctionality = function(thisPortList, pagedLink) {
			...
			if(nextPage <= maxNumPages || maxNumPages === 0){
				...
				var ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'entropia_core_portfolio_ajax_load_more');
