...
class EdgeSkin extends EntropiaEdgeClassSkinAbstract {
    /**
     * Skin constructor. Hooks to entropia_edge_admin_scripts_init and entropia_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('entropia_edge_action_admin_scripts_init', array($this, 'registerStyles'));
        add_action('entropia_edge_action_admin_scripts_init', array($this, 'registerScripts'));

        add_action('entropia_edge_action_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('entropia_edge_action_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('entropia_edge_action_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('entropia_edge_action_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			entropia_edge_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            entropia_edge_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = entropia_edge_get_admin_tab();
        $active_page = $entropia_edge_global_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see EntropiaEdgeClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param EntropiaEdgeClassAdminPage $page current page to load
     * @see EntropiaEdgeClassSkinAbstract::loadTemplatePart()
     */
