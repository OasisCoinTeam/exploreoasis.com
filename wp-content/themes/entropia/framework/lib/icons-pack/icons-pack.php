...
foreach ( glob( EDGE_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( entropia_edge_is_customizer_item_enabled( $module_load, 'entropia_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'entropia_edge_get_icon_pack_collections' ) ) {
	function entropia_edge_get_icon_pack_collections() {
		$options = apply_filters( 'entropia_edge_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'entropia_edge_get_icon_pack_collections' );
}

if ( ! function_exists( 'entropia_edge_get_icon_pack_options' ) ) {
	function entropia_edge_get_icon_pack_options() {
		$options = apply_filters( 'entropia_edge_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'entropia_edge_get_icon_pack_options' );
}

if ( ! function_exists( 'entropia_edge_get_icon_pack_social_options' ) ) {
	function entropia_edge_get_icon_pack_social_options() {
		$options = apply_filters( 'entropia_edge_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'entropia_edge_get_icon_pack_social_options' );
}

/*
 * Class: EntropiaEdgeClassIconCollections
 */
class EntropiaEdgeClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = entropia_edge_get_icon_pack_collections();
		$this->iconPackOptions       = entropia_edge_get_icon_pack_options();
		$this->iconPackSocialOptions = entropia_edge_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = entropia_edge_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'entropia' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'entropia' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'entropia' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'entropia' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'entropia' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'entropia' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'entropia' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'entropia' )       => 'fa-lg',
			esc_html__( 'Small', 'entropia' )      => 'fa-2x',
			esc_html__( 'Medium', 'entropia' )     => 'fa-3x',
			esc_html__( 'Large', 'entropia' )      => 'fa-4x',
			esc_html__( 'Very Large', 'entropia' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iEntropiaEdgeInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'entropia' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'entropia' ) ] = '';
