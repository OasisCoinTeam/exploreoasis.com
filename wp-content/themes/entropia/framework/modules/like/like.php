...
class EntropiaEdgeClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_entropia_edge_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_entropia_edge_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'entropia' );
		
		if ( isset( $_COOKIE[ 'edgtf-like_' . $post->ID ] ) ) {
			$class = 'edgtf-like liked';
			$title = esc_html__( 'You already like this!', 'entropia' );
		}
		
		return '<a href="#" class="' . esc_attr( $class ) . '" id="edgtf-like-' . esc_attr( $post->ID ) . '-' . $rand_number . '" title="' . esc_attr( $title ) . '">' . $output . '</a>';
	}
}

if ( ! function_exists( 'entropia_edge_create_like' ) ) {
	function entropia_edge_create_like() {
		EntropiaEdgeClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'entropia_edge_create_like' );
}