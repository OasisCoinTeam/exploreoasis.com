...
class EntropiaEdgeClassSocialIconWidget extends EntropiaEdgeClassWidget {
	
	public function __construct() {
		parent::__construct(
			'edgtf_social_icon_widget',
			esc_html__( 'Entropia Social Icon Widget', 'entropia' ),
			array( 'description' => esc_html__( 'Add social network icons to widget areas', 'entropia' ) )
		);
		...
		$this->params = array_merge(
			array(
				array(
					'type'  	  => 'dropdown',
					'name'  	  => 'icon_text',
					'title' 	  => esc_html__( 'Display Icon or Text', 'entropia' ),
					'description' => esc_html__( 'Choose whether you want to display social icon or text instead', 'entropia' ),
					'options' 	  => array(
						'icon' => esc_html__( 'Icon', 'entropia' ),
						'text' => esc_html__( 'Text', 'entropia' )
					)
				),
				array(
					'type'        => 'textfield',
					'name'        => 'social_text',
					'title'       => esc_html__( 'Social Text', 'entropia' ),
					'description' => esc_html__( 'Instert text which will describe desired social network', 'entropia' )
				)
			),
			entropia_edge_icon_collections()->getSocialIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'link',
	...
	public function widget( $args, $instance ) {
		...
		?>
		<a class="edgtf-social-icon-widget-holder edgtf-icon-has-hover" <?php echo entropia_edge_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php entropia_edge_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
