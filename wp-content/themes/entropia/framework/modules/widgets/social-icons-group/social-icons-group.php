...
class EntropiaEdgeClassClassIconsGroupWidget extends EntropiaEdgeClassWidget {
	public function __construct() {
		parent::__construct(
			'edgtf_social_icons_group_widget',
			esc_html__( 'Entropia Social Icons Group Widget', 'entropia' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'entropia' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			entropia_edge_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'entropia' ),
					'options' => array(
						''             => esc_html__( 'Default', 'entropia' ),
						'square-icons' => esc_html__( 'Square', 'entropia' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'entropia' ),
					'description' => esc_html__( 'This applies to the square layout', 'entropia' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'entropia' ),
						'light-skin' => esc_html__( 'Light Skin', 'entropia' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'entropia' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'entropia' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="edgtf-social-icon-widget-holder edgtf-icon-has-hover" <?php echo entropia_edge_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php entropia_edge_inline_style( $icon_styles ) ?>
