...
namespace EntropiaCore\CPT\Shortcodes\MasonryGallery;
...
class MasonryGallery implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Masonry Gallery', 'entropia-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Items', 'entropia-core' )
						),
						array(
							'type'        => 'dropdown',
						...
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'entropia-core' ),
			...
			$html .= '<div class="edgtf-mg-inner edgtf-outer-space">';
				...
				if ( $query->have_posts() ) :
					while ( $query->have_posts() ) : $query->the_post();
						...
						$html .= entropia_core_get_cpt_shortcode_module_template_part( 'masonry-gallery', 'masonry-gallery-list', 'masonry-gallery-' . $type . '-template', '', $params );
					
					endwhile;
				else:
					$html .= esc_html__( 'Sorry, no posts matched your criteria.', 'entropia-core' );
				endif;
				wp_reset_postdata();
			$html .= '</div>';
		$html .= '</div>';
	...
	private function getItemClasses() {
		...
		$background_skin = get_post_meta( $itemID, 'edgtf_masonry_gallery_simple_content_background_skin', true );
		...
		if ( ! empty( $image_size ) ) {
			$classes[] = 'edgtf-masonry-size-' . $image_size;
		}
		
		if ( ! empty( $background_skin ) ) {
			$classes[] = 'edgtf-mg-skin-' . $background_skin;
		}
	...
	public function getItemImage( $params ) {
		...
		if ( ! empty( $imageOption ) ) {
			...
			$image_id  = entropia_edge_get_attachment_id_from_url( $image_url );
	...
	public function masonryGalleryCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['masonry_gallery_category_title'] ) > 0 ) ? esc_html__( 'Category', 'entropia-core' ) . ': ' . $value['masonry_gallery_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $masonry_gallery_category ) ) {
				...
				if ( ! empty( $masonry_gallery_category_title ) ) {
					$masonry_gallery_category_title_display = esc_html__( 'Category', 'entropia-core' ) . ': ' . $masonry_gallery_category_title;
				}
