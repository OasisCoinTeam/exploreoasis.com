<div class="edgtf-reviews-list-info edgtf-reviews-per-mark">
	<?php foreach ( $post_ratings as $rating ) { ?>
		<?php
		$average_rating     = entropia_core_post_average_rating( $rating );
		$rating_count       = $rating['count'];
		$rating_count_label = $rating_count === 1 ? esc_html__( 'Rating', 'entropia-core' ) : esc_html__( 'Ratings', 'entropia-core' );
				...
				<div class="edgtf-reviews-number-wrapper">
					...
					<span class="edgtf-stars-wrapper">
                        ...
                        <span class="edgtf-reviews-count">
                            <?php esc_html_e( 'Rated', 'entropia-core' ) . ' ' . $average_rating . ' ' . esc_html__( 'out of', 'entropia-core' ) . ' ' . $rating_count . ' ' . $rating_count_label; ?>
                        </span>
                    </span>
				</div>
			</div>
			<div class="edgtf-grid-col-8">
				<div class="edgtf-rating-percentage-wrapper">
					...
					foreach ( $rating_marks as $item => $value ) {
						...
						echo do_shortcode( '[edgtf_progress_bar percent="' . esc_attr( $percentage ) . '" title="' . esc_attr( $item ) . esc_html__( ' stars', 'entropia-core' ) . '"]' );
					}
					?>
				</div>
			</div>
