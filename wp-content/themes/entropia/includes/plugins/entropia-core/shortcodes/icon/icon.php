<?php
namespace EntropiaCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'entropia-core' ),
				    'base'                      => $this->base,
					    ...
					    array(
						    array(
							    ...
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'entropia-core' )   => 'edgtf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'entropia-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'entropia-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'entropia-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Enable Icon Shadow', 'entropia-core' ),
							    'value'      => array(
								    esc_html__( 'No', 'entropia-core' ) => '',
								    esc_html__( 'Yes', 'entropia-core' ) => 'edgtf-icon-shadow'
							    ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgtf-normal' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'icon_shadow_color',
							    'heading'    => esc_html__( 'Shadow Color', 'entropia-core' ),
							    'dependency' => array( 'element' => 'icon_shodow', 'value' => array( 'edgtf-icon-shadow' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'entropia-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'edgtf-circle', 'edgtf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'entropia-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'entropia-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'entropia-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = entropia_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconHolderClasses( $params ) {
		...
		$holderClasses[] = ! empty( $params['icon_shadow'] ) ? $params['icon_shadow'] : '';
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'edgtf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'edgtf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . entropia_edge_filter_px( $params['custom_size'] ) . 'px';
			}
		}

		if ( $params['type'] == 'edgtf-normal' && $params['icon_shadow'] == 'edgtf-icon-shadow' ) {
			if ( ! empty( $params['icon_shadow_color'] ) ) {

				$iconStyles[] = 'text-shadow: 0 0 4px' . $params['icon_shadow_color'] . ', 0 0 5px' . $params['icon_shadow_color'];
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'edgtf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . entropia_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . entropia_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . entropia_edge_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . entropia_edge_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'edgtf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . entropia_edge_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
